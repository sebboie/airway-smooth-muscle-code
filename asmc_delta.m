...
y1     = 100.0; % Cao et al. 
y2     = 10.0; % Cao et al. 
kIPR   = 0.07; % 0.07 Cao et al. 0.05
k24    = 0.35; % Cao et al.
km24   = 80.0; % Cao et al.

% Parameters for serca pump
Vs     = 13.25; %13.25; % Cao et al. 10.0
ns     = 1.75; % Cao et al.
Ks     = 0.26; % Cao et al.
...
% Ion concentrations
K_ext  = 8; % Roux et al. 5
K_int  = 120; % Roux et al. 150
...
y(2,:) = y1.*(JIPR-Jdiff);
y(3,:) = open.*scale.*(-Jpm+influx.*(-Jrocc-(0.0406-Delta.*0.0229)/0.0177.*Jsocc-Delta.*J_CaL));
y(4,:) = lh42.*(km42.^3./(cb.^3+km42.^3)-h42);
y(5,:) = 1.0./Tso.*(Ksocc.^4./(Ksocc.^4+cs.^4)-Pso);
y(6,:) = -1.0./Cm.*(open.*(Jpm+influx.*(Jrocc+(0.0406-Delta.*0.0229)/0.0177.*Jsocc+Delta.*J_CaL))+I_Kdr+I_ClCa+I_bK+I_bNa+I_NaK+I_KCa);
end
