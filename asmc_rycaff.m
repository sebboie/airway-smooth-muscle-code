function y = asmc_rycaff(t, x, par)
...
% Ryanodine receptor
aux  = max(pry,(kryr0+kryr1.*c.^3./(kryr2.^3+c.^3)).*(cs.^4./(kryr3.^4.0+cs.^4.0)));
if aux < 1
    PRyR = aux;
    y(7,:) = 0.05*PRyR;
...
y(1,:) = Jdiff + Jleak + JRyR - Jserca + open.*scale.*(-Jpm+influx.*(-Jrocc-Jsocc-J_CaL));
y(2,:) = y1.*(JIPR-Jdiff);
